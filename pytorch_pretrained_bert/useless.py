...
class BertEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.mix_at = [10,11]
        self.layer[10] = BertLayerMix(config)
        self.layer[11] = BertEntLayer(config)
    def forward(self, hidden_states, attention_mask,kc_entity_one_hop_ids,kc_entity_one_hop_types,
                kc_entity_two_hop_labels,kc_entity_two_hop_types,
        ...
        kc_two_hop_entity_hiddens = entity_embedding(kc_entity_two_hop_labels)
        kc_two_hop_type_hiddens = entity_type_embedding(kc_entity_two_hop_types)

        for index,layer_module in enumerate(self.layer):
            if(index not in self.mix_at): # 
                hidden_states = layer_module(hidden_states, attention_mask)
            else:
                hidden_states, one_hop_entity_hiddens = layer_module(hidden_states, attention_mask,
                                                                kc_entity_one_hop_ids,kc_entity_two_hop_labels,
                                                                kc_entity_one_hop_types,kc_entity_two_hop_types,
                                                                kc_one_hop_entity_hiddens,kc_one_hop_type_hiddens,
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None,
                ...
                # two_hop_entity_ids = None,  two_hop_entity_types = None,
                kc_entity_two_hop_labels=None,kc_entity_two_hop_types=None,
...
class BertEntLayer(nn.Module):
    ...
    def two_hops_entity_enhanced(self,
        ...
        two_hop_num_factor = torch.where( two_hop_num_factor==0 ,torch.ones_like(two_hop_num_factor),two_hop_num_factor)
...
class ERNIEForPreTraining(BertPreTrainedModel):
    ...
    #     two_hop_ent_mask = two_hop_entity_ids.ne_(-1)
    #     return one_hop_input_ent, one_hop_input_entity_type, one_hop_ent_mask, two_hop_input_ent, two_hop_ent_mask
    def forward(self, input_ids, segment_ids, input_mask, masked_lm_labels, kc_entity_one_hop_ids,
                kc_entity_one_hop_types, next_sentence_label, # two_hop_entity_ids, two_hop_entity_types,
                kc_entity_se_index, kc_entity_two_hop_labels, kc_entity_out_or_in, kc_entity_two_hop_rel_types,kc_entity_two_hop_types):
                #,two_hop_entity_type_mask, two_hop_entity_type_index_select):
        # the id in ent_labels should be consistent with the order of candidate. input_ent: 这里是经过mask机制的实体embedding【带有噪声的实体embedding】, 缺省的为0，其他的部分是对应到trans系列预训练过后的结果。mask表示这个位置如果没有匹配到实体，就为0，匹配到了就是1

        # 在GPU中补全模型所需其他的参数：既可以加快CPU计算，减少GPU等待，也可以较少传入参数的数量
        # one_hop_input_ent, one_hop_input_entity_type, one_hop_ent_mask, two_hop_input_ent, two_hop_ent_mask = \
        #     self.parameter_complete(one_hop_entity_ids, one_hop_entity_types, two_hop_entity_ids)
        
        sequence_output, pooled_output = self.bert(input_ids, segment_ids, input_mask, kc_entity_one_hop_ids,
                                                   kc_entity_one_hop_types,kc_entity_two_hop_labels,kc_entity_two_hop_types
                                                #    two_hop_entity_ids,two_hop_entity_types,
        ...
        if masked_lm_labels is not None and next_sentence_label is not None:
            ...
            fin_span_rep = fin_span_rep/ (torch.where(norm!=0,norm,torch.ones_like(norm)))
            # print(self.entity_classifer.size())
            # print(torch.norm(self.entity_classifer,p=None,dim=-1).size())
            # assert(False)
            norm = torch.norm(self.entity_classifer,p=None,dim=-1,keepdim=True)
            classifer = self.entity_classifer / (torch.where(norm!=0,norm,torch.ones_like(norm)))
            # check_has_nan(fin_span_rep)
            # check_has_nan(classifer)
            # assert(torch.sum(torch.isnan())==0)
            
            entity_logist = torch.matmul(fin_span_rep[:,:-1],classifer.t())
            
            # entity_score = 
            # print(entity_logist[0])
            # print(kc_entity_two_hop_labels[0])
            
            entity_loss = loss_fct(entity_logist,kc_entity_two_hop_labels.view(-1))
